(ns ratp-paris.main
  "Kotodama WASM entrypoint for the Ratp_paris clean-room actor (L4) — Clojure port.
  ...
  facts (`ratp_paris.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ratp_paris")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ratp_paris.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ratp_paris-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ratp_paris-compat/ready)
